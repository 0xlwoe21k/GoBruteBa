...
func (s *Source) enumerate(ctx context.Context, searchURL string, domainRegexp *regexp.Regexp, tokens *Tokens, session *subscraping2.Session, results chan subscraping2.Result) {
	select {
	case <-ctx.Done():
		return
	default:
...
func proccesItems(ctx context.Context, items []item, domainRegexp *regexp.Regexp, name string, session *subscraping2.Session, results chan subscraping2.Result) error {
		...
		if resp.StatusCode == http.StatusOK {
			...
			for scanner.Scan() {
				...
				for _, subdomain := range domainRegexp.FindAllString(normalizeContent(line), -1) {
					results <- subscraping2.Result{Source: name, Type: subscraping2.Subdomain, Value: subdomain}
				}
			}
			resp.Body.Close()
		}
