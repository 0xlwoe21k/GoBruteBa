...
func (a *agent) enumerate(ctx context.Context, baseURL string) {
	...
	if err != nil && !isnotfound {
		a.results <- subscraping2.Result{Source: "sitedossier", Type: subscraping2.Error, Error: err}
	...
	for _, match := range a.session.Extractor.FindAllString(src, -1) {
		a.results <- subscraping2.Result{Source: "sitedossier", Type: subscraping2.Subdomain, Value: match}
	}
