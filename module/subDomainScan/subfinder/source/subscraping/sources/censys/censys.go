	...
	go func() {
		...
		for {
			...
			resp, err := session.HTTPRequest(
				...
				subscraping2.BasicAuth{Username: session.Keys.CensysToken, Password: session.Keys.CensysSecret},
			)
			...
			for _, res := range censysResponse.Results {
				for _, part := range res.Data {
					results <- subscraping2.Result{Source: s.Name(), Type: subscraping2.Subdomain, Value: part}
				}
