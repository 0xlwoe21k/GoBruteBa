...
func (s *Source) Run(ctx context.Context, domain string, session *subscraping2.Session) <-chan subscraping2.Result {
	...
	go func() {
		...
		if baseURL == "" {
			results <- subscraping2.Result{Source: s.Name(), Type: subscraping2.Error, Error: fmt.Errorf("can't get API URL")}
			return
		}
...
func (s *Source) enumerate(ctx context.Context, session *subscraping2.Session, baseURL string, page int, authHeader map[string]string, results chan subscraping2.Result) {
	...
	if response.Message != "" && response.Status != nil {
		results <- subscraping2.Result{Source: s.Name(), Type: subscraping2.Error, Error: fmt.Errorf(response.Message)}
	}
...
func isV2(ctx context.Context, session *subscraping2.Session, authHeader map[string]string) bool {
