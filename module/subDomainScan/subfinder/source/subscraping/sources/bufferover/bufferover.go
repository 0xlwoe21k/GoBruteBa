...
func (s *Source) getData(ctx context.Context, sourceURL string, session *subscraping2.Session, results chan subscraping2.Result) {
	...
	if len(metaErrors) > 0 {
		results <- subscraping2.Result{Source: s.Name(), Type: subscraping2.Error, Error: fmt.Errorf("%s", strings.Join(metaErrors, ", "))}
		return
	}
	...
	for _, subdomain := range subdomains {
		for _, value := range session.Extractor.FindAllString(subdomain, -1) {
			results <- subscraping2.Result{Source: s.Name(), Type: subscraping2.Subdomain, Value: value}
		}
	}
}
