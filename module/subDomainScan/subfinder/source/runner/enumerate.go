...
func (r *Runner) EnumerateSingleDomain(ctx context.Context, domain string, outputs []io.Writer) error {
	...
	var resolutionPool *resolve2.ResolutionPool
	...
	uniqueMap := make(map[string]resolve2.HostEntry)
	...
	go func() {
		for result := range passiveResults {
			...
			case subscraping2.Subdomain:
				...
				hostEntry := resolve2.HostEntry{Host: subdomain, Source: result.Source}
	...
	foundResults := make(map[string]resolve2.Result)
	if r.options.RemoveWildcard {
